...
from trainer_on_crf import BaseLSTMCRFTrainer
...
if __name__ == "__main__":
	...
	# model=M.load_embedding(model)
	model=torch.load('./pkl/0612_multi_long_model_lstm_onembedding7.pkl')
	# torch.save(model.embeddings,'pkl/embeddings0607.pkl')
	# model2=M.LSTMLSTM(
	# 	model=model,
	# 	)
	# model2=torch.load('./pkl/0604_multi_long_model_lstmcrf1.pkl')
	...
	# model=M.reset_parameters(model)
	...
	accuracy, precision, recall, f1score=get_perf('./result/0610_long_pred_lstmlstm_ON_embedding.txt')
	# torch.save(model,'./pkl/multi_long_model_lstmcrf.pkl')
	# f=open('result.txt','w')
	# i=0
	# while i < 71:
	# 	model=torch.load('./pkl/0609_multi_long_model_lstm_onembedding'+str(i)+'.pkl')
	# 	trainer=BaseLSTMCRFTrainer(
	# 		model=model,
	# 		epochs=epoch,
	# 		input_vocabs=input_vocabs,
	...
	# 		)
	# 	accuracy, precision, recall, f1score=get_perf('./result/0610_long_pred_lstmlstm_ON_embedding.txt')
	# 	f.write(str(i)+','+str(accuracy)+','+str(f1score)+'\n')
	# 	i+=5
