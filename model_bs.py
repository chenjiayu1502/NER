...
class LSTMCRF(nn.Module):
    ...
    def predict_for_bs(self, xs, lens, trans,return_scores=False,k=5):
        ...
        for i in range(xs.size(1)):#
            ...
            for j in range(lenss[i]):
                # print('j==',j)
                label_list=self.get_former_label(res)
                # print(label_list)
                for former_label in label_list:
                    top_k_p,top_k=self.beam_search(output,j,k,former_label,trans)
                    res=self.extend_res(res,top_k_p,top_k,j,former_label)
