...
class BaseLSTMCRFTrainer(object):
    ...
    def write_result(self,pred,targ):
        ...
        f=open('./result/multi_long_0604_lstmcrf_bio_pred.txt','w')
        f2=open('./result/multi_long_0604_lstmcrf_bio_targ.txt','w')
        assert len(pred)==len(targ)
        for i in range(len(pred)):
            assert len(pred[i])==len(targ[i])
            f.write(' '.join(pred[i])+'\n')
    ...
    def write_result2(self,sen,pred,targ):
        print('writing_result...........')
        f=open('./result/0610_long_pred_lstmlstm_ON_embedding.txt','w')
        assert len(pred)==len(targ)==len(sen)
        for i in range(len(pred)):
            assert len(pred[i])==len(targ[i])==len(sen[i])
            for j in range(len(pred[i])):
                f.write(' '.join([sen[i][j],pred[i][j],targ[i][j]])+'\n')
        f.close()
    ...
    def train(self,data, data_size):
        ...
        for e_idx in range(self.epochs):
            ...
            for i_idx, (batch, lens) in enumerate(data):
                ...
                loglik, logits = M.loglik(self.model,xs_var, y_var, lens_s)
                nll = loglik#.mean()


                nll_v = nll#float(-(loglik / lens_s.float()).data[0])
                nll_v_all+=nll_v.data.numpy().tolist()[0]
                nll.backward()
                self.optimizer.step()

                #预测结果
                # preds = self.model.predict(xs_var, lens_s)
                preds = M.predict(self.model, xs_var, lens_s)
                #print(preds[0])
            ...
            torch.save(self.model,'./pkl/0612_multi_long_model_lstm_onembedding'+str(e_idx)+'.pkl')
    ...
    def test(self,data, data_size):
        ...
        #print(y_var_all[-1-e_idx])
        print('acc=',acc) 
        self.write_result2(x_var_all,preds_all,y_var_all)
